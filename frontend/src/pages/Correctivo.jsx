...
const Correctivo = () => {
  ...
  const handleFinish = async () => {
    const hasPlanilla = mantenimiento.planilla !== undefined && mantenimiento.planilla !== null && mantenimiento.planilla !== '';
  ...
  const handleSaveInfo = async (e) => {
    ...
    if (formData.extendido !== undefined) {
      // ver punto 4 para permitir limpiar
      if (formData.extendido) formDataToSend.append("extendido", formData.extendido);
      else formDataToSend.append("extendido", ""); // si tu backend interpreta vacío como limpiar
    }
    ...
    if (formData.estado) {
      ...
      if ((formData.estado === "Finalizado" || formData.estado === "Solucionado") && !mantenimiento.fecha_cierre) {
        const now = new Date();
        const yyyy = now.getFullYear();
        const mm = String(now.getMonth() + 1).padStart(2, "0");
        const dd = String(now.getDate()).padStart(2, "0");
        formDataToSend.append("fecha_cierre", `${yyyy}-${mm}-${dd}`);
      }
    }
    ...
    try {
      ...
      setError(error.response?.data?.detail || "Error al actualizar la información.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSavePlanilla = async (e) => {
    ...
    if (!formData.planilla) {
      setError("Seleccione una planilla para guardar.")
      return
    }
    ...
    try {
      await updateMantenimientoCorrectivo(mantenimiento.id, formDataToSend)
      setSuccess("Planilla guardada correctamente.")
      setFormData({ ...formData, planilla: "" })
      setPlanillaPreview("")
      await fetchMantenimiento()
    } catch (error) {
      console.error("Error saving planilla:", error)
      setError(error.response?.data?.detail || "Error al guardar la planilla.")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleSavePhotos = async (e) => {
    ...
    if (formData.fotos.length === 0) {
      setError("Seleccione fotos para guardar.")
      return
    }
    ...
    try {
      ...
      setError(error.response?.data?.detail || "Error al guardar las fotos.")
    } finally {
      setIsLoading(false)
    }
  }
